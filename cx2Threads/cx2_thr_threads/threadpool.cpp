...
using namespace CX2::Threads::Pool;
...
TasksQueue *ThreadPool::getRandomTaskQueueWithElements()
{
    ...
    mutexRandom.lock();
    CX2::Helpers::Random::safe_random_shuffle(fullVector.begin(), fullVector.end(),static_cast<size_t>(dis(lRand)));
    mutexRandom.unlock();
...
size_t ThreadPool::getRandomQueueByKey(const std::string &key, const float &priority)
{
    ...
    for (size_t i=0; i<queues.size(); ++i) fullVector.push_back(i);
    CX2::Helpers::Random::safe_random_shuffle(fullVector.begin(), fullVector.end(), hash_fn(key));
