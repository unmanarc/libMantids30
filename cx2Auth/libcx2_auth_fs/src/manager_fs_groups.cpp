...
bool Manager_FS::_pGroupExist(const std::string &groupName)
{
...
bool Manager_FS::_pGroupDirCreate(const std::string &groupName, std::string &groupsDirOut)
{
...
bool Manager_FS::_pGroupDir(const std::string &groupName, std::string &groupDirOut)
{
...
bool Manager_FS::_pGroupAccountsDir(const std::string &groupDir, std::string &groupAccountsOut)
{
    groupAccountsOut = groupDir + "/accounts";
    return !access(groupAccountsOut.c_str(), W_OK);
}

bool Manager_FS::_pGroupAttribsDir(const std::string &groupDir, std::string &groupAttribsOut)
{
    groupAttribsOut = groupDir + "/attribs";
    return !access(groupAttribsOut.c_str(), W_OK);
}

bool Manager_FS::_pGroupsDir( std::string &accountsDirOut)
{
    if (access(workingAuthDir.c_str(), W_OK)) return false;
...
std::set<std::string> Manager_FS::groupsList()
{
...
bool Manager_FS::groupAdd(const std::string &groupName, const std::string &groupDescription)
{
...
bool Manager_FS::groupRemove(const std::string &groupName)
{
    std::string groupDir;
    bool r = false;
    Threads::Sync::Lock_RW lock(mutex);
...
bool Manager_FS::groupExist(const std::string &groupName)
{
    bool r = false;
    Threads::Sync::Lock_RD lock(mutex);
    r = _pGroupExist(groupName);
...
bool Manager_FS::groupChangeDescription(const std::string &groupName, const std::string &groupDescription)
{
    bool r = true;
    std::string groupDir;
    Threads::Sync::Lock_RW lock(mutex);
...
std::string Manager_FS::groupDescription(const std::string &groupName)
{
    std::string r;
    std::string groupDir;
    Threads::Sync::Lock_RD lock(mutex);
...
std::set<std::string> Manager_FS::groupAccounts(const std::string &groupName, bool lock)
{
...
std::string Manager_FS::getGroupDetailsFilePath(const std::string &groupName)
{
