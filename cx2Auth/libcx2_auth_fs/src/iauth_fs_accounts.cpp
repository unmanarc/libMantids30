...
bool IAuth_FS::_pAccountExist(const std::string &accountName)
{
...
bool IAuth_FS::_pAccountDirCreate(const std::string &accountName, std::string &accountDirOut)
{
...
bool IAuth_FS::_pAccountDir(const std::string &accountName, std::string &accountDirOut)
{
...
bool IAuth_FS::_pAccountsDir( std::string &accountsDirOut)
{
    if (access(workingAuthDir.c_str(), W_OK)) return false;
...
std::set<std::string> IAuth_FS::accountsList()
{
                          ...
                          bool superuser)
{
    ...
    if (!workingAuthDir.empty())
    {
        ...
        if (r)
        {
            ...
            Files::Vars::File fAcctPassword(getPasswordFilePath(accountName,0));
            Files::Vars::File fAcctActivation(getActivationFilePath(accountName));

            for (const auto & i : passData.getMap()) fAcctPassword.addVar(i.first,i.second);
            ...
            if (r && (r = fAcctPassword.save())) r = true;
...
bool IAuth_FS::accountRemove(const std::string & accountName)
{
    bool r = false;
    std::string accountDir;
    Threads::Sync::Lock_RW lock(mutex);
...
bool IAuth_FS::accountDisable(const std::string &accountName, bool disabled)
{
    bool r = false;
    std::string accountDir;
    Threads::Sync::Lock_RW lock(mutex);
...
bool IAuth_FS::accountConfirm(const std::string &accountName, const std::string &confirmationToken)
{
    bool r = false;
    std::string accountDir;
    Threads::Sync::Lock_RW lock(mutex);
...
bool IAuth_FS::accountChangePassword(const std::string &accountName, const DataStructs::sPasswordData &passwordData, uint32_t passIndex)
{
    bool r = false;
    std::string accountDir;
    Threads::Sync::Lock_RW lock(mutex);
    ...
    if (!workingAuthDir.empty())
    {
        if (_pAccountDir(accountName,accountDir))
        {
            Files::Vars::File f(getPasswordFilePath(accountName,passIndex));
...
bool IAuth_FS::accountChangeDescription(const std::string &accountName, const std::string &description)
{
    bool r = false;
    std::string accountDir;
    Threads::Sync::Lock_RW lock(mutex);
...
bool IAuth_FS::accountChangeEmail(const std::string &accountName, const std::string &email)
{
    bool r = false;
    std::string accountDir;
    Threads::Sync::Lock_RW lock(mutex);
...
bool IAuth_FS::accountChangeExtraData(const std::string &accountName, const std::string &extraData)
{
    bool r = false;
    std::string accountDir;
    Threads::Sync::Lock_RW lock(mutex);
...
bool IAuth_FS::accountChangeExpiration(const std::string &accountName, time_t expiration)
{
    bool r = false;
    std::string accountDir;
    Threads::Sync::Lock_RW lock(mutex);
...
bool IAuth_FS::isAccountDisabled(const std::string &accountName)
{
    bool r = true;
    std::string accountDir;
    Threads::Sync::Lock_RD lock(mutex);
...
bool IAuth_FS::isAccountConfirmed(const std::string &accountName)
{
    bool r = false;
    std::string accountDir;
    Threads::Sync::Lock_RD lock(mutex);
...
bool IAuth_FS::isAccountSuperUser(const std::string &accountName)
{
    bool r = false;
    std::string accountDir;
    Threads::Sync::Lock_RD lock(mutex);
...
std::string IAuth_FS::accountDescription(const std::string &accountName)
{
    std::string r;
    std::string accountDir;
...
std::string IAuth_FS::accountEmail(const std::string &accountName)
{
    std::string r;
    std::string accountDir;
...
DataStructs::sPasswordData IAuth_FS::retrievePassword(const std::string &accountName, uint32_t passIndex, bool *found)
{
...
std::string IAuth_FS::accountConfirmationToken(const std::string & accountName)
{
...
std::string IAuth_FS::accountExtraData(const std::string &accountName)
{
    std::string r;
    std::string accountDir;
...
time_t IAuth_FS::accountExpirationDate(const std::string &accountName)
{
    time_t r = 0;
    std::string accountDir;
...
std::string IAuth_FS::getActivationFilePath(const std::string &accountName)
{
    return (workingAuthDir + "/accounts/" + CX2::Helpers::Encoders::toURL(accountName) + "/acct.activation");
}

std::string IAuth_FS::getAccountDetailsFilePath(const std::string &accountName)
{
    return (workingAuthDir  + "/accounts/" + CX2::Helpers::Encoders::toURL(accountName) + "/acct.details");
}

std::string IAuth_FS::getPasswordFilePath(const std::string &accountName, uint32_t passIndex)
{
