...
struct sVolatileAuth_Account
{
    sVolatileAuth_Account()
    {
        superuser = false;
        enabled = false;
        expirationDate = 0;
    ...
    std::map<uint32_t,DataStructs::sPasswordData> passwordByIDX;
    std::set<std::string> accountGroups;
    std::set<std::string> accountAttribs;
};

struct sVolatileAuth_Group
{
    std::string name,description;
    std::set<std::string> groupAttribs;
};

struct sVolatileAuth_Attrib
{
    std::string name,description;
};
...
class IAuth_Volatile : public IAuth
{
public:
...
private:
    std::map<std::string,DataStructs::sVolatileAuth_Group> groups;
    std::map<std::string,DataStructs::sVolatileAuth_Account> accounts;
    std::map<std::string,DataStructs::sVolatileAuth_Attrib> attribs;
};
