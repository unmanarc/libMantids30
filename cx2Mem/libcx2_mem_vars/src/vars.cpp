...
{
    CX2::Memory::Containers::B_Base * value = getValue(varName);
...
std::list<std::string> Vars::getStringValues(const std::string &varName)
{
    ...
    std::list<CX2::Memory::Containers::B_Base *> contList = getValues(varName);
    for (CX2::Memory::Containers::B_Base * b : contList)
