...
std::pair<bool,uint64_t> B_MEM::copyTo2(Streamable &bc, Streams::Status & wrStatUpd, const uint64_t &roBytes, const uint64_t &offset)
{
    uint64_t bytes = roBytes;
    // No bytes to copy.
    if (!bytes) return std::make_pair(true,0);
...
bool B_MEM::compare2(const void *buf, const uint64_t &len, bool caseSensitive, const uint64_t &offset)
{
    ...
    return !CX2::Helpers::Mem::memicmp2(memoryToBeCompared, buf, len,caseSensitive);
}
