...
#include <cx2_xrpc_common/json_streamable.h>
...
eHTTP_RetCode WebClientHandler::processClientRequest()
{
             ...
             || getLocalFilePathFromURI(resourcesLocalPath, &sRealRelativePath, &sRealFullPath, ".html") )
    {
        ...
        log(LEVEL_INFO,hSession, "fileServer", 2048, "R/%03d: %s",HTTP_Status::getHTTPRetCodeValue(ret),sRealRelativePath.c_str());
        log(LEVEL_DEBUG,hSession, "fileServer", 2048, "R LOCAL/%03d: %s",HTTP_Status::getHTTPRetCodeValue(ret),sRealFullPath.c_str());
    }
    else
    {
...
eHTTP_RetCode WebClientHandler::processHTMLIEngine( const std::string & sRealFullPath,WebSession * hSession )
{
...
eHTTP_RetCode WebClientHandler::processRPCRequest()
{
    ...
    if (csrfValidationOK)
    {
        ...
        // CHANGE MY ACCOUNT PASSWORD...
                 ...
                 webSession->authSession->getIsFullyLoggedIn(CX2::Authentication::CHECK_ALLOW_EXPIRED_PASSWORDS) && // All required login passwords passed the authorization
    ...
    if (bCloseSessionHandler)
    {
        ...
        HTTP_Cookie simpleJSSecureCookie;
        ...
        simpleJSSecureCookie.setSameSite(HTTP_COOKIE_SAMESITE_STRICT);
...
eHTTP_RetCode WebClientHandler::processRPCRequest_AUTHINFO(WebSession *wSession, const uint32_t & uMaxAge)
{
    Memory::Streams::JSON_Streamable * jPayloadOutStr = new Memory::Streams::JSON_Streamable;
    jPayloadOutStr->setFormatted(useFormattedJSONOutput);
...
eHTTP_RetCode WebClientHandler::processRPCRequest_INITAUTH(const Authentication & auth, string sSessionId)
{
    CX2::Authentication::Reason authReason;
    //CX2::Authentication::Session *authSession = nullptr;
    uint64_t uMaxAge;
    Memory::Streams::JSON_Streamable * jPayloadOutStr = new Memory::Streams::JSON_Streamable;
    jPayloadOutStr->setFormatted(useFormattedJSONOutput);
...
eHTTP_RetCode WebClientHandler::processRPCRequest_POSTAUTH(const Authentication &auth, WebSession *currentWebSession, bool * destroySession)
{
    CX2::Authentication::Reason authReason;
    CX2::Authentication::Session *authSession = currentWebSession->authSession;
    Memory::Streams::JSON_Streamable * jPayloadOutStr = new Memory::Streams::JSON_Streamable;
    jPayloadOutStr->setFormatted(useFormattedJSONOutput);
    eHTTP_RetCode eHTTPResponseCode = HTTP_RET_401_UNAUTHORIZED;
...
eHTTP_RetCode WebClientHandler::processRPCRequest_EXEC(WebSession * wSession, MultiAuths *extraAuths)
{
    CX2::Authentication::Session *authSession = nullptr;
    ...
    if (!authDomains)
    {
        ...
        return HTTP_RET_500_INTERNAL_SERVER_ERROR;
    }
    ...
    {
        CX2::Authentication::Reason authReason=temporaryAuthentication( userName,
                                                                        domainName,
                                                                        extraAuths->getAuthentication(passIdx) );

        // Include the pass idx in the Extra TMP Index.
        if ( CX2::Authentication::IS_PASSWORD_AUTHENTICATED( authReason ) )
        {
            log(LEVEL_INFO, wSession, "rpcServer", 2048, "Adding valid in-execution authentication factor {method=%s,idx=%d,reason=%s}", sMethodName.c_str(),passIdx,CX2::Authentication::getReasonText(authReason));
            extraTmpIndexes.insert(passIdx);
        }
        else
        {
            log(LEVEL_WARN, wSession, "rpcServer", 2048, "Rejecting invalid in-execution authentication factor {method=%s,idx=%d,reason=%s}", sMethodName.c_str(),passIdx,CX2::Authentication::getReasonText(authReason));
        }
    }
    ...
    if (authorizer)
    {
        ...
        case VALIDATION_NOTAUTHORIZED:
        {
            ...
            log(LEVEL_ERR, wSession, "rpcServer", 8192, "Not authorized to execute method {method=%s,reasons=%s}", sMethodName.c_str(),Memory::Streams::JSON_Streamable::jsonToString(reasons).c_str());
            eHTTPResponseCode = HTTP_RET_401_UNAUTHORIZED;
        }break;
...
eHTTP_RetCode WebClientHandler::processRPCRequest_CHPASSWD(const Authentication &oldAuth, WebSession *wSession, bool * destroySession)
{
    CX2::Authentication::Session *authSession = wSession->authSession;
    eHTTP_RetCode eHTTPResponseCode = HTTP_RET_401_UNAUTHORIZED;
    Memory::Streams::JSON_Streamable * jPayloadOutStr = new Memory::Streams::JSON_Streamable;
    jPayloadOutStr->setFormatted(useFormattedJSONOutput);
    ...
    if (domainAuthenticator)
    {
        ...
        clientDetails.sUserAgent = requestData().USER_AGENT;
        ...
        if (IS_PASSWORD_AUTHENTICATED(authReason))
        {
            ...
            CX2::Authentication::Secret newSecretData = CX2::Authentication::createNewSecret(newAuth.getPassword(),CX2::Authentication::FN_SSHA256);
        ...
        {
            log(LEVEL_ERR, wSession, "rpcServer", 2048, "Password change failed, bad incomming credentials {idx=%d,reason=%s}",credIdx,CX2::Authentication::getReasonText(authReason));
...
eHTTP_RetCode WebClientHandler::processRPCRequest_TESTPASSWD(const Authentication &auth, WebSession *wSession, bool * destroySession)
{
    CX2::Authentication::Session *authSession = wSession->authSession;
    eHTTP_RetCode eHTTPResponseCode = HTTP_RET_401_UNAUTHORIZED;
    Memory::Streams::JSON_Streamable * jPayloadOutStr = new Memory::Streams::JSON_Streamable;
    ...
    if (domainAuthenticator)
    {
        ...
        {
            log(LEVEL_ERR, wSession, "rpcServer", 2048, "Password validation failed, bad incomming credentials {idx=%d,reason=%s}",auth.getPassIndex(),CX2::Authentication::getReasonText(authReason));
...
eHTTP_RetCode WebClientHandler::processRPCRequest_PASSWDLIST(WebSession *wSession)
{
    CX2::Authentication::Session *authSession = wSession->authSession;
    eHTTP_RetCode eHTTPResponseCode = HTTP_RET_200_OK;
    Memory::Streams::JSON_Streamable * jPayloadOutStr = new Memory::Streams::JSON_Streamable;
    jPayloadOutStr->setFormatted(useFormattedJSONOutput);
    ...
    if (domainAuthenticator)
    {
        std::map<uint32_t, CX2::Authentication::Secret_PublicData> publics = domainAuthenticator->getAccountAllSecretsPublicData(authSession->getAuthUser());
        ...
        eHTTPResponseCode = HTTP_RET_500_INTERNAL_SERVER_ERROR;
...
std::string WebClientHandler::persistentAuthentication(const string &userName, const string &domainName, const Authentication &authData, CX2::Authentication::Session *authSession, CX2::Authentication::Reason * authReason)
{
    ...
    {
        *authReason = CX2::Authentication::REASON_INTERNAL_ERROR;
        return sessionId;
    }
    ...
    *authReason = CX2::Authentication::REASON_INVALID_DOMAIN;
    ...
    if (domainAuthenticator)
    {
        ...
        *authReason = domainAuthenticator->authenticate(appName,clientDetails,userName,authData.getPassword(),authData.getPassIndex(), CX2::Authentication::MODE_PLAIN,"",&stAccountPassIndexesUsedForLogin);
    ...
    if ( CX2::Authentication::IS_PASSWORD_AUTHENTICATED( *authReason ) )
    {
        ...
        if (!authSession)
        {
            authSession = new CX2::Authentication::Session(appName);
...
CX2::Authentication::Reason WebClientHandler::temporaryAuthentication( const std::string & userName, const std::string & domainName,const Authentication &authData)
{
...
string WebClientHandler::getAuthSessionID(CX2::Authentication::Session *authSession)
{
