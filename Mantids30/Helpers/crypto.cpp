...
std::shared_ptr<Mem::BinaryDataContainer> Crypto::AES256DecryptB64ToBin(const std::string &input, const char *key, const size_t &keyLen)
{
    bool ok;
    ...
    if (dec->data && dec->cur >= 32)
    {
        ...
        if (PKCS5_PBKDF2_HMAC(key, safe_cast_or<int>(keyLen,0), salt, sizeof(salt), 100000, EVP_sha256(), sizeof(derivedKey), derivedKey) == 1)
        {
            ...
            if ((err = EVP_DecryptInit_ex(ctx, EVP_aes_256_gcm(), nullptr, derivedKey, salt)) == 1)
            {
                ...
                {
                    int len = -1;
                    if (EVP_DecryptUpdate(ctx, static_cast<unsigned char *>(r->data), &len, static_cast<unsigned char *>(dec->data) + 32, dec->cur - 32) == 1 && len >= 0)
                    {
                        r->cur += len;

                        if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_TAG, 16, reinterpret_cast<void *>(gcmTag)))
                        {
...
std::string Crypto::calcSHA1(const std::string &password)
{
    ...
    EVP_DigestUpdate(ctx, reinterpret_cast<const unsigned char *>(password.c_str()), password.length());
