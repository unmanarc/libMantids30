...
std::optional<boost::property_tree::ptree> Mantids30::Program::Config::Loader::loadSecureApplicationConfig(Mantids30::Program::Logs::AppLog * log, const std::string &dir, const std::string &filePath)
{
    ...
    {
        log->log0(__func__, Program::Logs::LEVEL_WARN, "The configuration '%s' file is inaccessible, loading defaults...", filePath.c_str());
    }
    else
    {
        if (isConfigFileInsecure)
        {
            log->log0(__func__, Program::Logs::LEVEL_SECURITY_ALERT,
                          ...
                          "update any affected applications or services as necessary.",filePath.c_str());

            if (Helpers::File::fixSensitiveConfigPermission(filePath))
            {
                log->log0(__func__, Program::Logs::LEVEL_SECURITY_ALERT, "The permissions of the '%s' file has been changed to 0600.",filePath.c_str());
            }
            else
            {
                log->log0(__func__, Program::Logs::LEVEL_CRITICAL, "The permissions of the '%s' file can't be changed.",filePath.c_str());
                return std::nullopt;
            }
        }

        boost::property_tree::info_parser::read_info(filePath, pConfig);
    }
