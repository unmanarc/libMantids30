...
bool Query_MariaDB::step0()
{
    ...
    for (size_t col = 0; col < m_resultVars.size(); col++)
    {
        ...
        std::shared_ptr<Memory::Abstract::BINARY::ByteArray> sBin = nullptr;
        ...
        case Memory::Abstract::Var::TYPE_BIN:
        {
            ...
            if (varSize > 0)
            {
                sBin = std::make_shared<Memory::Abstract::BINARY::ByteArray>(varSize);
        ...
        case Memory::Abstract::Var::TYPE_VARCHAR:
        {
            ...
            result.length = ABSTRACT_PTR_AS(VARCHAR, outputVar)->getFillSizePTR();
            result.buffer = outputVar->getDirectMemory();
...
{
    return (m_lastSQLErrno == CR_CONN_HOST_ERROR || m_lastSQLErrno == CR_SERVER_GONE_ERROR || m_lastSQLErrno == CR_CONNECTION_ERROR || m_lastSQLErrno == CR_SERVER_LOST
            || m_lastSQLErrno == CR_UNKNOWN_HOST || m_lastSQLErrno == 1927);
}
...
bool Query_MariaDB::exec0(const ExecType &execType, bool recursion)
{
    ...
    if ((m_lastSQLReturnValue = mysql_stmt_prepare(m_stmt, m_query.c_str(), m_query.size())) != 0)
    {
        ...
        {
            throw std::runtime_error("Error preparing the statement: " + m_lastSQLError);
        }
    ...
    if (mysql_stmt_bind_param(m_stmt, m_bindedInputParams))
    {
        ...
        if (m_throwCPPErrorOnQueryFailure)
        {
            throw std::runtime_error("Error binding the parameters: " + m_lastSQLError);
        }
        return false;
    }
    ...
    if ((m_lastSQLReturnValue = mysql_stmt_execute(m_stmt)) != 0)
    {
        ...
        if (m_throwCPPErrorOnQueryFailure)
        {
            throw std::runtime_error("Error executing the query: " + m_lastSQLError);
        }
        return false;
    }
    ...
    if (mysql_stmt_store_result(m_stmt) != 0)
    {
        ...
        if (m_throwCPPErrorOnQueryFailure)
        {
            throw std::runtime_error("Error storing the results: " + m_lastSQLError);
        }
        return false;
    }
