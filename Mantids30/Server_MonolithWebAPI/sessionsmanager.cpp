...
bool WebSessionsManager::destroySession(const std::string &sessionID)
{
    ...
    {
        std::unique_lock<std::mutex> lock(m_mutex);
        if (m_sessionClientInfo.find(effectiveUser) == m_sessionClientInfo.end() || m_sessionClientInfo[effectiveUser].find(sessionID) == m_sessionClientInfo[effectiveUser].end())
        {
...
json WebSessionsManager::getUserSessionsInfo(const std::string &effectiveUserName)
{
    ...
    for (const std::pair<std::string, json> &sessions : sessionsMap)
    {
