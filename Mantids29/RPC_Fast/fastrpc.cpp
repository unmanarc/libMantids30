...
bool FastRPC::addMethod(const std::string &methodName, const FastRPC::Method &rpcMethod)
{
...
void FastRPC::eventUnexpectedAnswerReceived(FastRPC::Connection *, const std::string & )
{
}

int FastRPC::processAnswer(FastRPC::Connection * connection)
{
...
int FastRPC::processQuery(Network::Sockets::Socket_Stream_Base *stream, const std::string &key, const float &priority, Threads::Sync::Mutex_Shared * mtDone, Threads::Sync::Mutex * mtSocket, void *obj, const std::string &data)
{
    uint32_t maxAlloc = maxMessageSize;
    uint64_t requestId;
    char * payloadBytes;
    ...
    // Process / Inject task:
    Mantids29::Helpers::JSONReader2 reader;
    FastRPC::ThreadParameters * params = new FastRPC::ThreadParameters;
...
{
    FastRPC::Connection * connection =(FastRPC::Connection *)connectionsByKeyId.openElement(connectionKey);
...
int FastRPC::processConnection(Network::Sockets::Socket_Stream_Base *stream, const std::string &key, const FastRPC::CallBackOnConnected &_cb_OnConnected, const float &keyDistFactor, void *obj, const std::string &data)
{
#ifndef _WIN32
    ...
    FastRPC::Connection * connection = new FastRPC::Connection;
    connection->obj = obj;
...
{
    FastRPC::ThreadParameters * params = (FastRPC::ThreadParameters *)(taskData);
...
void FastRPC::sendRPCAnswer(FastRPC::ThreadParameters *params, const std::string &answer,uint8_t execution)
{
    // Send a block.
...
json FastRPC::runRemoteRPCMethod(const std::string &connectionKey, const std::string &methodName, const json &payload, json *error, bool retryIfDisconnected)
{
    ...
    FastRPC::Connection * connection;

    uint32_t _tries=0;
    while ( (connection=(FastRPC::Connection *)connectionsByKeyId.openElement(connectionKey))==nullptr )
    {
        _tries++;
...
bool FastRPC::runRemoteClose(const std::string &connectionKey)
{
    ...
    FastRPC::Connection * connection;
    if ((connection=(FastRPC::Connection *)connectionsByKeyId.openElement(connectionKey))!=nullptr)
    {
...
void FastRPC::eventFullQueueDrop(FastRPC::ThreadParameters *)
{
}
