...
using namespace Mantids29::Threads::Pool;
...
ThreadPool::TasksQueue *ThreadPool::getRandomTaskQueueWithElements()
{
    ...
    mutexRandom.lock();
    Mantids29::Helpers::Random::safe_random_shuffle(fullVector.begin(), fullVector.end(),static_cast<size_t>(dis(lRand)));
    mutexRandom.unlock();
...
size_t ThreadPool::getRandomQueueByKey(const std::string &key, const float &priority)
{
    ...
    for (size_t i=0; i<queues.size(); ++i) fullVector.push_back(i);
    Mantids29::Helpers::Random::safe_random_shuffle(fullVector.begin(), fullVector.end(), hash_fn(key));
