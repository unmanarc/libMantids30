...
void RPCClientImpl::runRPClient()
{
    ...
    for (;;)
    {
        Mantids29::Network::Sockets::Socket_TLS sockRPCClient;
        ...
        if (!Globals::getLC_C2UsePSK())
        {
            ...
            if (  !Globals::getLC_TLSPhraseFileForPrivateKey().empty() )
            {
                bool ok = false;
                // Load Key
                std::string keyPassPhrase = Mantids29::Helpers::Crypto::AES256DecryptB64( Mantids29::Helpers::File::loadFileIntoString( Globals::getLC_TLSPhraseFileForPrivateKey() )
...
RPCClientImpl::PSKIdKey RPCClientImpl::loadPSK()
{
    bool ok = false;
    PSKIdKey r;
    std::string encryptedKey = Mantids29::Helpers::File::loadFileIntoString( Globals::getLC_C2PSKSharedKeyFilePath() , &ok);
    auto masterKey = Globals::getMasterKey();
    ...
    {
        std::string tokenizedKey = Mantids29::Helpers::Crypto::AES256DecryptB64( encryptedKey,(char *)masterKey->data,masterKey->length,&ok );
