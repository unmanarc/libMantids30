...
struct sAccountDetails{
    std::string sGivenName,sLastName,sEmail,sDescription,extraData;
};
...
public:
     ...
     */
    virtual Secret_PublicData getAccountSecretPublicData(const std::string & sAccountName, uint32_t passIndex=0) override;
    ...
    virtual bool validateAccountAttribute(const std::string & sAccountName, const ApplicationAttribute & applicationAttrib) override;
    ...
    virtual std::set<ApplicationAttribute> accountDirectAttribs(const std::string & sAccountName, bool lock = true)=0;
    std::set<ApplicationAttribute> accountUsableAttribs(const std::string & sAccountName);
    virtual time_t accountExpirationDate(const std::string & sAccountName)=0;

    virtual void updateLastLogin(const std::string &sAccountName, const uint32_t & passwordIndex, const ClientDetails & clientDetails)=0;
    ...
    virtual bool attribAccountAdd(const ApplicationAttribute & applicationAttrib, const std::string & sAccountName)=0;
    virtual bool attribAccountRemove(const ApplicationAttribute & applicationAttrib, const std::string & sAccountName, bool lock = true)=0;
...
protected:
    virtual bool accountValidateDirectAttribute(const std::string & sAccountName, const ApplicationAttribute & applicationAttrib)=0;
