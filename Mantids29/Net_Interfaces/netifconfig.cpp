#include <Mantids29/Memory/a_ipv4.h>
...
using namespace Mantids29::Network::Interfaces;
...
bool NetworkInterfaceConfiguration::apply()
{
    ...
    if (changeIPv4Addr)
    {
        auto i = Mantids29::Helpers::AppExec::blexec(createNetSHCMD({"interface",
                                                               "ipv4",
                                                               "set",
                                                               "address",
                                                               std::string("name="+std::to_string(adapterIndex)),
                                                               "static",
                                                               Mantids29::Memory::Abstract::IPV4::_toString(address),
                                                               Mantids29::Memory::Abstract::IPV4::_toString(netmask)
                                                              }));
        if (i.error==0)
            changeIPv4Addr = false;
        else
...
Mantids29::Helpers::AppExec::sAppExecCmd NetworkInterfaceConfiguration::createRouteCMD(const std::vector<std::string> &routecmdopts)
{
    Mantids29::Helpers::AppExec::sAppExecCmd x;
    x.arg0 = getRouteExecPath();
    x.args = routecmdopts;
    return x;
}

Mantids29::Helpers::AppExec::sAppExecCmd NetworkInterfaceConfiguration::createNetSHCMD(const std::vector<std::string> &netshcmdopts)
{
    Mantids29::Helpers::AppExec::sAppExecCmd x;
