...
Mantids29::Helpers::AppExec::sAppExecResult Mantids29::Helpers::AppExec::blexec(const sAppExecCmd &cmd)
{
    sAppExecResult rt;
...
Mantids29::Helpers::AppSpawn::AppSpawn()
{
    attrp = nullptr;
...
bool Mantids29::Helpers::AppSpawn::setExec(const std::string &path)
{
    if (
        #ifndef _WIN32
...
bool Mantids29::Helpers::AppSpawn::addOpenFDToFile(const std::string &outFile, int fd)
{
    return posix_spawn_file_actions_addopen (file_actionsp, fd, outFile.c_str(),
                                             O_WRONLY | O_CREAT | O_TRUNC, 0644)==0;
}

bool Mantids29::Helpers::AppSpawn::redirectStdErrToStdOut()
{
    return posix_spawn_file_actions_adddup2 (file_actionsp, STDOUT_FILENO, STDERR_FILENO)==0;
}

void Mantids29::Helpers::AppSpawn::addArgument(const std::string &arg)
{
    arguments.push_back(arg);
}

void Mantids29::Helpers::AppSpawn::addEnvironment(const std::string &env)
{
    environment.push_back(env);
}

bool Mantids29::Helpers::AppSpawn::spawnProcess(bool pipeStdout, bool pipeStderr)
{
    // Create the argv...
...
void Mantids29::Helpers::AppSpawn::waitUntilProcessEnds()
{
    int s,status;
    do
    {
...
std::set<int> Mantids29::Helpers::AppSpawn::pollResponse()
{
    std::set<int> r;
...
ssize_t Mantids29::Helpers::AppSpawn::read(int fd, void *buf, size_t count)
{
    if (fd == STDOUT_FILENO)
