...
std::set<uint32_t> Manager_DB::passIndexesRequiredForLogin()
{
    ...
    Abstract::UINT32 idx;
    std::shared_ptr<SQLConnector::QueryInstance> i = m_sqlConnector->qSelect("SELECT `index` FROM vauth_v4_secretsindexs WHERE `loginRequired`=:loginRequired;",
...
bool Manager_DB::passIndexAdd(const uint32_t &passIndex, const std::string &description, const bool &loginRequired)
{
    ...
    return m_sqlConnector->query("INSERT INTO vauth_v4_secretsindexs (`index`,`indexDescription`,`loginRequired`) "
...
{
    Threads::Sync::Lock_RW lock(mutex);
    return m_sqlConnector->query("UPDATE vauth_v4_secretsindexs SET `loginRequired`=:loginRequired, `indexDescription`=:indexDescription WHERE `index`=:index;",
                               {
                                   {":index",             new Memory::Abstract::UINT32(passIndex)},
                                   {":indexDescription",  new Memory::Abstract::STRING(description)},
                                   {":loginRequired",     new Memory::Abstract::BOOL(loginRequired)}
...
{
    Threads::Sync::Lock_RW lock(mutex);
    return m_sqlConnector->query("DELETE FROM vauth_v4_secretsindexs WHERE `index`=:index;",
                               {
                                   {":index", new Memory::Abstract::UINT32(passIndex)}
                               });
}
...
std::string Manager_DB::passIndexDescription(const uint32_t &passIndex)
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = m_sqlConnector->qSelect("SELECT `indexDescription` FROM vauth_v4_secretsindexs WHERE `index`=:index LIMIT 1;",
                                          {  {":index",             new Memory::Abstract::UINT32(passIndex)} },
                                          { &description });
...
bool Manager_DB::passIndexLoginRequired(const uint32_t &passIndex)
{
    ...
    std::shared_ptr<SQLConnector::QueryInstance> i = m_sqlConnector->qSelect("SELECT `loginRequired` FROM vauth_v4_secretsindexs WHERE `index`=:index LIMIT 1;",
                                          {  {":index",             new Memory::Abstract::UINT32(passIndex)} },
                                          { &loginRequired });
