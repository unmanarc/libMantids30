...
{
    if (!m_sqlConnector->dbTableExist("vauth_v3_attribsaccounts"))
    {
        bool r =
                m_sqlConnector->query("CREATE TABLE `vauth_v3_accounts` (\n"
                                    ...
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_applications` (\n"
                                    ...
                                    "       PRIMARY KEY(`appName`)\n"
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_attribs` (\n"
                                    ...
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_applicationmanagers` (\n"
                                    ...
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_applicationusers` (\n"
                                    ...
                                    "       FOREIGN KEY(`f_appName`)  REFERENCES vauth_v3_applications(`appName`) ON DELETE CASCADE\n"
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_secretsindexs` (\n"
                                    "       `index`                 INTEGER       NOT NULL,\n"
                                    "       `indexDescription`      VARCHAR(4096) NOT NULL,\n"
                                    "       `loginRequired`         BOOLEAN       NOT NULL,\n"
                                    "       PRIMARY KEY(`index`)\n"
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_accountlogins` (\n"
                                    "       `f_userName`        VARCHAR(256)    NOT NULL,\n"
                                    ...
                                    "       FOREIGN KEY(`f_userName`)       REFERENCES vauth_v3_accounts(`userName`) ON DELETE CASCADE\n"
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_accountmanagers` (\n"
                                    "       `f_userNameManager`     VARCHAR(256)    NOT NULL,\n"
                                    ...
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_accountactivationtokens` (\n"
                                    "       `f_userName`            VARCHAR(256) NOT NULL,\n"
                                    "       `confirmationToken`     VARCHAR(256) NOT NULL,\n"
                                    "       PRIMARY KEY(`f_userName`),\n"
                                    "       FOREIGN KEY(`f_userName`) REFERENCES vauth_v3_accounts(`userName`) ON DELETE CASCADE\n"
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_accountsecrets` (\n"
                                    "       `f_secretIndex`         INTEGER         NOT NULL,\n"
                                    "       `f_userName`            VARCHAR(256)    NOT NULL,\n"
                                    ...
                                    "       FOREIGN KEY(`f_userName`)         REFERENCES vauth_v3_accounts(`userName`) ON DELETE CASCADE\n"
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE \"vauth_v3_groups\" (\n"
                                    "       `groupName`             VARCHAR(256) NOT NULL,\n"
                                    "       `groupDescription`           VARCHAR(4096),\n"
                                    "       PRIMARY KEY(`groupName`)\n"
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_groupsaccounts` (\n"
                                    "       `f_groupName`           VARCHAR(256) NOT NULL,\n"
                                    "       `f_userName`            VARCHAR(256) NOT NULL,\n"
                                    "       FOREIGN KEY(`f_groupName`)      REFERENCES vauth_v3_groups(`groupName`) ON DELETE CASCADE,\n"
                                    "       FOREIGN KEY(`f_userName`)       REFERENCES vauth_v3_accounts(`userName`) ON DELETE CASCADE\n"
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_attribsgroups` (\n"
                                    "       `f_appName`             VARCHAR(256) NOT NULL,\n"
                                    ...
                                    "       FOREIGN KEY(`f_groupName`)              REFERENCES vauth_v3_groups(`groupName`) ON DELETE CASCADE\n"
                                    ");\n") &&
                m_sqlConnector->query("CREATE TABLE `vauth_v3_attribsaccounts` (\n"
                                    "       `f_appName`             VARCHAR(256) NOT NULL,\n"
                                    "       `f_attribName`          VARCHAR(256) NOT NULL,\n"
                                    "       `f_userName`            VARCHAR(256) NOT NULL,\n"
                                    "       FOREIGN KEY(`f_appName`,`f_attribName`) REFERENCES vauth_v3_attribs(`f_appName`,`attribName`) ON DELETE CASCADE,\n"
                ...
                m_sqlConnector->query("CREATE UNIQUE INDEX `idx_groups_accounts` ON `vauth_v3_groupsaccounts` (`f_groupName` ,`f_userName`);\n") &&
                m_sqlConnector->query("CREATE UNIQUE INDEX `idx_attribs_groups` ON `vauth_v3_attribsgroups` (`f_appName`,`f_attribName`,`f_groupName` );\n") &&
                m_sqlConnector->query("CREATE UNIQUE INDEX `idx_attribs_accounts` ON `vauth_v3_attribsaccounts` (`f_appName`,`f_attribName`,`f_userName`);\n") &&
                m_sqlConnector->query("INSERT INTO vauth_v3_secretsindexs (`index`,`indexDescription`,`loginRequired`) "
