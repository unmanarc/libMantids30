...
WebServer::WebServer()
{
    ...
    obj = nullptr;
    sessionsManager.startGarbageCollector( SessionsManager::threadGC, &sessionsManager, "GC:WebSessions" );
...
void WebServer::acceptMultiThreaded(const std::shared_ptr<Network::Sockets::Socket_Stream_Base> & listenerSocket, const uint32_t &maxConcurrentConnections)
{
    ...
    multiThreadedAcceptor->setCallbackOnConnect(_onConnect,obj);
    multiThreadedAcceptor->setCallbackOnInitFail(_onInitFailed,obj);
    multiThreadedAcceptor->setCallbackOnTimedOut(_onTimeOut,obj);
...
void WebServer::acceptPoolThreaded(const std::shared_ptr<Network::Sockets::Socket_Stream_Base> & listenerSocket, const uint32_t &threadCount, const uint32_t &threadMaxQueuedElements)
{
    if (!methodManagers) throw std::runtime_error("Don't Accept XRPC Web before setting some methodsmanager");
    if (!authenticator) throw std::runtime_error("Don't Accept XRPC Web before setting some authenticator");
    ...
    poolThreadedAcceptor->setCallbackOnConnect(_onConnect,obj);
    poolThreadedAcceptor->setCallbackOnInitFail(_onInitFailed,obj);
    poolThreadedAcceptor->setCallbackOnTimedOut(_onTimeOut,obj);
...
bool WebServer::_onConnect(void * obj, Network::Sockets::Socket_Stream_Base * s, const char *cUserIP, bool isSecure)
{
...
bool WebServer::_onInitFailed(void * obj, Network::Sockets::Socket_Stream_Base * s, const char * cUserIP, bool isSecure)
{
    WebServer * webserver = ((WebServer *)obj);
...
void WebServer::_onTimeOut(void * obj, Network::Sockets::Socket_Stream_Base *s, const char * cUserIP, bool isSecure)
{
    WebServer * webserver = ((WebServer *)obj);
...
std::map<std::string, Mantids29::Memory::Containers::B_MEM *> WebServer::getStaticContentElements()
{
...
void WebServer::addInternalContentElement(const std::string &path, const std::string &content)
{
    ...
    if (staticContentElements.find(path) == staticContentElements.end())
    {
        ...
        staticContentElements[path] = new Mantids29::Memory::Containers::B_MEM(xmem,content.size());
...
Mantids29::RPC::Web::WebServer::sWebServerCallBack WebServer::getExtCallBackOnTimeOut() const
{
    return extCallBackOnTimeOut;
}

Mantids29::RPC::Web::WebServer::sWebServerCallBack WebServer::getExtCallBackOnInitFailed() const
{
    return extCallBackOnInitFailed;
}

Mantids29::RPC::Web::WebServer::sWebServerCallBack WebServer::getExtCallBackOnConnect() const
{
...
Mantids29::Authentication::Domains *WebServer::getAuthenticator() const
{
...
void WebServer::setAuthenticator(Mantids29::Authentication::Domains *value)
{
    authenticator = value;
}
