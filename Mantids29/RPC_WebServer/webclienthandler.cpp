...
void WebClientHandler::setAuthenticators(Mantids29::Authentication::Domains *authenticator)
{
    authDomains = authenticator;
}

Status::eRetCode WebClientHandler::procHTTPClientContent()
{
    ...
    // COOKIES:
    sSessionId = clientRequest.getCookie("sessionId");
    // TODO: filter invalid session id formats.
    // HEADERS:
    sClientCSRFToken = clientRequest.getHeaderOption("CSRFToken");

    // POST VARS / EXTRA AUTHS:
    if (!extraCredentials.setAuthentications(clientRequest.getVars(HTTP_VARS_POST)->getStringValue("extraAuth")))
        return HTTP::Status::S_400_BAD_REQUEST;

    // POST VARS / AUTH:
    if (!credentials.setJsonString(clientRequest.getVars(HTTP_VARS_POST)->getStringValue("auth")))
        return HTTP::Status::S_400_BAD_REQUEST;
    ...
    if (requestURI == "/api")
    {
        ...
        std::string mode = clientRequest.getVars(HTTP_VARS_GET)->getStringValue("mode");
        if (mode == "EXEC")
    ...
    if (requestURI == "/japi_exec") ret = procJAPI_Exec(&extraCredentials,
                                                        clientRequest.getVars(HTTP_VARS_GET)->getStringValue("method"),
                                                        clientRequest.getVars(HTTP_VARS_POST)->getStringValue("payload")
                                                        );
...
//      string tagClose     = string(whatStaticText[3].first, whatStaticText[3].second);
         ...
         start = fileContent.begin(), end =  fileContent.end()) // RESET AND RECHECK EVERYTHING
    {
        ...
        // Report as not found.
        if ( ! (authSession && authSession->doesSessionVariableExist(varName))  )
        {
            // look in post/get
            log(LEVEL_ERR, "fileserver", 2048, "Main variable not found: '%s' on resource '%s'",varName.c_str(),sRealFullPath.c_str());
            boost::replace_all(fileContent,fulltag, "null");
        ...
        {
            replaceTagByJVar(fileContent,fulltag,authSession->getSessionVariableValue(varName),false,scriptVarName);
        }
    }
...
Status::eRetCode WebClientHandler::procJAPI_Session()
{
    ...
    // GET VARS:
    sMode = clientRequest.getVars(HTTP_VARS_GET)->getStringValue("mode");

    // In AUTHCSRF we take the session ID from post variable (not cookie).
    if (sSessionId.empty() && usingCSRFToken && sMode == "AUTHCSRF")
    {
        sSessionId = clientRequest.getVars(HTTP_VARS_POST)->getStringValue("sessionId");
    ...
    if (csrfValidationOK)
    {
        ...
        if ( !webSession && sMode == "LOGIN" && (!usingCSRFToken || sClientCSRFToken == "00112233445566778899"))
            eHTTPResponseRetCode = procJAPI_Session_LOGIN(credentials);
        /////////////////////////////////////////////////////////////////
        // POST PERSISTENT AUTHENTICATION...
        else if (webSession && webSession->bAuthTokenConfirmed && sMode == "POSTLOGIN")
            eHTTPResponseRetCode = procJAPI_Session_POSTLOGIN(credentials);
        /////////////////////////////////////////////////////////////////
        // CHANGE MY ACCOUNT PASSWORD...
                 ...
                 authSession->isFullyAuthenticated(Mantids29::Authentication::Session::CHECK_ALLOW_EXPIRED_PASSWORDS) && // All required login passwords passed the authorization
...
Status::eRetCode WebClientHandler::procResource_File(Authentication::Multi *extraAuths)
{
         ...
         )
    {
        ...
        Mantids29::Authentication::Manager * authorizer = authSession?authDomains->openDomain(authSession->getAuthenticatedDomain()) : nullptr;
...
Status::eRetCode WebClientHandler::procJAPI_Version()
{
...
Status::eRetCode WebClientHandler::procJAPI_Session_AUTHINFO()
{
    Memory::Streams::StreamableJSON * jPayloadOutStr = new Memory::Streams::StreamableJSON;
    jPayloadOutStr->setFormatted(useFormattedJSONOutput);
...
Status::eRetCode WebClientHandler::procJAPI_Session_CSRFTOKEN()
{
    // On Page Load...
    Memory::Streams::StreamableJSON * jPayloadOutStr = new Memory::Streams::StreamableJSON;
    jPayloadOutStr->setFormatted(useFormattedJSONOutput);
...
Status::eRetCode WebClientHandler::procJAPI_Session_LOGIN(const Authentication::Data & auth)
{
    ...
    std::string user = clientRequest.getVars(HTTP_VARS_POST)->getStringValue("user");
    std::string domain = clientRequest.getVars(HTTP_VARS_POST)->getStringValue("domain");
    ...
    if (!sSessionId.empty())
    {
        ...
        if (currentWebSession)
        {
            if (!usingCSRFToken)
            {
                ...
                serverResponse.setSecureCookie("sessionId", sSessionId, uMaxAge);
            }
            else
            {
...
Status::eRetCode WebClientHandler::procJAPI_Session_POSTLOGIN(const Authentication::Data &auth)
{
    ...
    sSessionId = persistentAuthentication( authSession->getAuthUser(),
                                           authSession->getAuthenticatedDomain(),
                                           auth,
    ...
    {
        auto i = authSession->getNextRequiredAuthenticationIndex();
        if (i.first != 0xFFFFFFFF)
        {
            // No next login idx.
            (*(jPayloadOutStr->getValue())).removeMember("nextPassReq");
            ...
            log(LEVEL_INFO,  "rpcServer", 2048, "Authentication factor (%d) OK, waiting for the next authentication factor {val=%d,txt=%s}", auth.m_passwordIndex, i.first, i.second.c_str());
        }
        else
        {
            log(LEVEL_INFO,  "rpcServer", 2048, "Authentication factor (%d) OK, Logged in.", auth.m_passwordIndex);
        }
        eHTTPResponseRetCode = HTTP::Status::S_200_OK;
    }
    else
    {
        log(LEVEL_WARN,  "rpcServer", 2048, "Authentication error on factor #(%d), Logged out {val=%d,txt=%s}",auth.m_passwordIndex,
            JSON_ASUINT((*(jPayloadOutStr->getValue())),"val",0), JSON_ASCSTRING((*(jPayloadOutStr->getValue())),"txt","")
            );
                                                     ...
                                                     )
{
    ...
    if (authSession)
    {
        ...
        domainName = authSession->getAuthenticatedDomain();
    }

    if (!clientRequest.getVars(HTTP_VARS_POST)->getStringValue("payload").empty() && !reader.parse(sPayloadIn, jPayloadIn))
    {
    ...
    for (const uint32_t & passIdx : extraAuths->getAvailableIndices())
    {
        ...
        {
            log(LEVEL_INFO, "rpcServer", 2048, "Adding valid in-execution authentication factor {method=%s,idx=%d,reason=%s}", sMethodName.c_str(),passIdx,Mantids29::Authentication::getReasonText(authReason));
            extraTmpIndexes.insert(passIdx);
        }
        else
        {
            log(LEVEL_WARN, "rpcServer", 2048, "Rejecting invalid in-execution authentication factor {method=%s,idx=%d,reason=%s}", sMethodName.c_str(),passIdx,Mantids29::Authentication::getReasonText(authReason));
        }
    }
    ...
    if (authorizer)
    {
        ...
        case MethodsManager::VALIDATION_OK:
        {
            ...
            log(LEVEL_DEBUG, "rpcServer", 8192, "Executing Web Method - debugging parameters {method=%s,params=%s}", sMethodName.c_str(),Mantids29::Helpers::jsonToString(jPayloadIn).c_str());
            ...
            case Mantids29::RPC::MethodsManager::METHOD_RET_CODE_SUCCESS:
                ...
                log(LEVEL_DEBUG, "rpcServer", 8192, "Web Method executed OK - debugging parameters {method=%s,params=%s}", sMethodName.c_str(),Mantids29::Helpers::jsonToString(jPayloadOutStr->getValue()).c_str());
        ...
        case MethodsManager::VALIDATION_NOTAUTHORIZED:
        {
            ...
            log(LEVEL_ERR, "rpcServer", 8192, "Not authorized to execute method {method=%s,reasons=%s}", sMethodName.c_str(),Mantids29::Helpers::jsonToString(reasons).c_str());
            eHTTPResponseRetCode = HTTP::Status::S_401_UNAUTHORIZED;
        }break;
...
Status::eRetCode WebClientHandler::procJAPI_Session_CHPASSWD(const Authentication::Data &oldAuth)
{
    ...
    // POST VARS / AUTH:
    if (!newAuth.setJsonString(clientRequest.getVars(HTTP_VARS_POST)->getStringValue("newAuth")))
    {
    ...
    auto domainAuthenticator = authDomains->openDomain(authSession->getAuthenticatedDomain());
    if (domainAuthenticator)
    {
        Mantids29::Authentication::ClientDetails clientDetails;
        clientDetails.ipAddress = userIP;
        clientDetails.tlsCommonName = userTLSCommonName;
        clientDetails.userAgent = clientRequest.userAgent;

        auto authReason = domainAuthenticator->authenticate(appName,clientDetails,authSession->getAuthUser(),oldAuth.m_password,credIdx);
        ...
        if (IS_PASSWORD_AUTHENTICATED(authReason))
        {
            ...
            if ( JSON_ASBOOL((*(jPayloadOutStr->getValue())),"ok",false) == true)
                log(LEVEL_INFO, "rpcServer", 2048, "Password change requested {index=%d,result=1}",credIdx);
            else
                log(LEVEL_ERR, "rpcServer", 2048, "Password change failed due to internal error {index=%d,result=0}",credIdx);

            eHTTPResponseRetCode = HTTP::Status::S_200_OK;
        }
        else
        {
            log(LEVEL_ERR, "rpcServer", 2048, "Password change failed, bad incomming credentials {index=%d,reason=%s}",credIdx,Mantids29::Authentication::getReasonText(authReason));
        ...
        authDomains->releaseDomain(authSession->getAuthenticatedDomain());
    }
    else
    {
        log(LEVEL_ERR, "rpcServer", 2048, "Password change failed, domain authenticator not found {index=%d}",credIdx);
    }
...
Status::eRetCode WebClientHandler::procJAPI_Session_TESTPASSWD(const Authentication::Data &auth)
{
    ...
    if (!authSession) return eHTTPResponseRetCode;
    ...
    if (domainAuthenticator)
    {
        ...
        auto authReason = domainAuthenticator->authenticate(appName,clientDetails,authSession->getAuthUser(),auth.m_password,credIdx);
        if (IS_PASSWORD_AUTHENTICATED(authReason))
        {
            log(LEVEL_INFO, "rpcServer", 2048, "Password validation requested {index=%d,result=1}",auth.m_passwordIndex);
            //(*(jPayloadOutStr->getValue()))["ok"] = true;
            eHTTPResponseRetCode = HTTP::Status::S_200_OK;
        }
        else
        {
            log(LEVEL_ERR, "rpcServer", 2048, "Password validation failed, bad incomming credentials {index=%d,reason=%s}",auth.m_passwordIndex,Mantids29::Authentication::getReasonText(authReason));
    ...
    {
        log(LEVEL_ERR, "rpcServer", 2048, "Password validation failed, domain authenticator not found {index=%d}",auth.m_passwordIndex);
    }
...
Status::eRetCode WebClientHandler::procJAPI_Session_PASSWDLIST()
{
    HTTP::Status::eRetCode eHTTPResponseRetCode = HTTP::Status::S_401_UNAUTHORIZED;
    ...
    if (domainAuthenticator)
    {
        std::map<uint32_t, Mantids29::Authentication::Secret_PublicData> publics = domainAuthenticator->getAccountAllSecretsPublicData(authSession->getAuthUser());
...
std::string WebClientHandler::persistentAuthentication(const string &userName, const string &domainName, const Authentication::Data &authData, Mantids29::Authentication::Session *lAuthSession, Mantids29::Authentication::Reason * authReason)
{
    json payload;
    ...
    *authReason = Mantids29::Authentication::REASON_INVALID_DOMAIN;
    ...
    if (domainAuthenticator)
    {
        ...
        *authReason = domainAuthenticator->authenticate(appName,clientDetails,userName,authData.m_password,authData.m_passwordIndex, Mantids29::Authentication::MODE_PLAIN,"",&stAccountPassIndexesUsedForLogin);
    ...
    if ( Mantids29::Authentication::IS_PASSWORD_AUTHENTICATED( *authReason ) )
    {
        ...
        if (!lAuthSession)
        {
            lAuthSession = new Mantids29::Authentication::Session(appName);
...
Mantids29::Authentication::Reason WebClientHandler::temporaryAuthentication(const std::string & userName, const std::string & domainName, const Authentication::Data &authData)
{
    ...
    else
    {
        ...
        eReason = auth->authenticate( appName, clientDetails, userName,authData.m_password,authData.m_passwordIndex); // Authenticate in a non-persistent fashion.
        authDomains->releaseDomain(domainName);
    }
...
void WebClientHandler::log(eLogLevels logSeverity,  const std::string & module, const uint32_t &outSize, const char *fmtLog,...)
{
    ...
    if (rpcLog) rpcLog->logVA( logSeverity,
                               ...
                               !authSession?"" : authSession->getAuthenticatedDomain(),
