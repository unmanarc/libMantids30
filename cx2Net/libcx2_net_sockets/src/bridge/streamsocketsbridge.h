...
public:
     ...
     */
    bool setPeer(unsigned char i, StreamSocket * s);
    /**
     * @brief GetPeer Get the Pipe Peers
     * @param i peer number (0 or 1)
     * @return Stream Socket Peer.
     */
    StreamSocket * getPeer(unsigned char i);
    /**
     ...
     */
    void setCustomPipeProcessor(StreamsSocketsBridge_Thread *value, bool deleteOnExit = false);

private:
    static void remotePeerThread(StreamSocketsBridge * stp);

    static void pipeThread(StreamSocketsBridge * stp);

    StreamsSocketsBridge_Thread *bridgeThreadPrc;
    StreamSocket * socket_peers[2];
