...
StreamSocketReader::StreamSocketReader()
{
...
unsigned char StreamSocketReader::readU8(bool* readOK)
{
    unsigned char rsp[1] =
    { 0 };
    if (readOK)
...
uint16_t StreamSocketReader::readU16(bool* readOK)
{
    uint16_t ret = 0;
    if (readOK)
        *readOK = true;
...
uint32_t StreamSocketReader::readU32(bool* readOK)
{
    uint32_t ret = 0;
    if (readOK)
        *readOK = true;
...
uint64_t StreamSocketReader::readU64(bool *readOK)
{
    uint64_t ret = 0;
    if (readOK)
        *readOK = true;
...
char* StreamSocketReader::readBlock32WAllocAndDelim(unsigned int* datalen,
