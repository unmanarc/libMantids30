...
bool Socket_Multiplexer::multiplexedSocket_sendLineConnectionAnswer(const DataStructs::sLineID &lineId, const DataStructs::eLineAcceptAnswerMSG &msg, const json &answerValue,
                                                                    const uint32_t &localLineWindow)
{
    if (noSendData)
        return false;
...
void Socket_Multiplexer::server_AcceptConnection_Callback(DataStructs::sLineID lineId, const uint32_t &remoteWindowSize, const json &connectionParams)
{
    ...
    if (cbServerConnectionFinished.callbackFunction)
        cbServerConnectionFinished.callbackFunction(cbServerConnectionFinished.obj, lineId.localLineId, ssock);
