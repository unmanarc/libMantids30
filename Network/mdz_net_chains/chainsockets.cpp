...
ChainSockets::ChainSockets(Mantids::Network::Streams::StreamSocket *_baseSocket, bool _deleteBaseSocketOnExit)
{
...
std::pair<Mantids::Network::Streams::StreamSocket *, Mantids::Network::Streams::StreamSocket *> ChainSockets::getSocketPairLayer(size_t layer)
{
    std::pair <Mantids::Network::Streams::StreamSocket *, Mantids::Network::Streams::StreamSocket *> bar;
...
bool ChainSockets::addToChain(SocketChainBase *chainElement, bool deleteAtExit)
{
    return addToChain( chainElement->makeSocketChainPair(),
                             deleteAtExit, // If it's not a  detached pointer, declare as false, if it's detached true (when ChainSockets die, then, this chain element will die too)
...
bool ChainSockets::addToChain(std::pair<Mantids::Network::Streams::StreamSocket *, Mantids::Network::Streams::StreamSocket *> sockPairs, bool deleteFirstSocketOnExit, bool deleteSecondSocketOnExit, bool modeServer, bool detached, bool endPMode)
{
...
bool ChainSockets::isConnected()
{
    if (socketLayers.size() == 0 && !baseSocket) return false;
    Mantids::Network::Streams::StreamSocket * curSocket = !socketLayers.size()? baseSocket : ((sChainVectorItem *)socketLayers[socketLayers.size()-1])->sock[0];
...
int ChainSockets::partialRead(void *data, const uint32_t & datalen)
{
    if (socketLayers.size() == 0 && !baseSocket) return -1;
    Mantids::Network::Streams::StreamSocket * curSocket = !socketLayers.size()? baseSocket : ((sChainVectorItem *)socketLayers[socketLayers.size()-1])->sock[0];
...
int ChainSockets::partialWrite(const void *data, const uint32_t &datalen)
{
    if (socketLayers.size() == 0 && !baseSocket) return -1;
    Mantids::Network::Streams::StreamSocket * curSocket = !socketLayers.size()? baseSocket : ((sChainVectorItem *)socketLayers[socketLayers.size()-1])->sock[0];
...
void ChainSockets::chainThread(sChainTElement *chain)
{
    char data[4096];
    int r0;
    bool w1=0;
